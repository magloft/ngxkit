...
import { RequestSubscriptionOptions, WEB_PUSH_ID } from './module'
import { NgxPushSubscription, NgxPushSubscriptionOptionsInit, PushManagerBase } from './PushManagerBase'
import { PushManagerChrome } from './PushManagerChrome'
import { PushManagerSafari } from './PushManagerSafari'
...
export class SwPush {
  ...
  private pushManager: Observable<PushManagerBase>
   ...
   */
  requestSubscription(options: RequestSubscriptionOptions): Promise<NgxPushSubscription> {
    if (!this.sw.isEnabled) { return Promise.reject(new Error(ERR_SW_NOT_SUPPORTED)) }
    const pushOptions: NgxPushSubscriptionOptionsInit = {}
    if (this.isBrowserSafari()) {
      pushOptions.url = options.url
      pushOptions.userInfo = options.userInfo
    } else {
      let key = this.decodeBase64(options.serverPublicKey.replace(/_/g, '/').replace(/-/g, '+'))
      let applicationServerKey = new Uint8Array(new ArrayBuffer(key.length))
      for (let i = 0; i < key.length; i++) { applicationServerKey[i] = key.charCodeAt(i) }
      pushOptions.userVisibleOnly = true
      pushOptions.applicationServerKey = applicationServerKey
    }
    if (this._pushManager) {
      return this._pushManager.subscribe(pushOptions).then((sub) => {
  ...
  unsubscribe(): Promise<void> {
    ...
    const doUnsubscribe = (sub: NgxPushSubscription | null) => {
      if (sub === null) { throw new Error('Not subscribed to push notifications.') }
